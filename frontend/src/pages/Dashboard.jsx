...
const Dashboard = () => {
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <div className={`${iconBgLight} dark:bg-opacity-20 p-2.5 rounded-lg`}>
            ...
            {percentage !== undefined && (
              <div className="flex items-center gap-1 mt-2">
                <span className={`text-xs font-semibold px-2 py-1 rounded-full ${
                  trend === 'up' ? 'bg-green-50 dark:bg-green-900 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-400'
                }`}>
                  {trend === 'up' ? '↑' : '↓'} {percentage}%
                </span>
  ...
  <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-300 dark:border-gray-700 p-3">
    <div className="flex justify-between items-start">
      ...
      {/* Right Section */}
      {selectedAccount && (
        <div className="text-right bg-gray-50 dark:bg-gray-800 rounded-lg p-3 border border-gray-300 dark:border-gray-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Product Status Distribution</h2>
          </div>
          <div className="space-y-5">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-base font-bold text-green-600 dark:text-green-400">{stats.approvedProducts}</span>
              </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-base font-bold text-amber-600 dark:text-amber-400">{stats.pendingProducts}</span>
              </div>
              <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-2.5">
                <div
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-green-50 dark:bg-green-900 rounded-xl border border-green-100 dark:border-green-800">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-700 dark:text-gray-300 font-medium">Approved Products</span>
              </div>
              <span className="text-2xl font-bold text-green-600 dark:text-green-400">{stats.approvedProducts.toLocaleString()}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-red-50 dark:bg-red-900 rounded-xl border border-red-100 dark:border-red-800">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-700 dark:text-gray-300 font-medium">Disapproved Products</span>
              </div>
              <span className="text-2xl font-bold text-red-600 dark:text-red-400">{stats.disapprovedProducts.toLocaleString()}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-amber-50 dark:bg-amber-900 rounded-xl border border-amber-100 dark:border-amber-800">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-700 dark:text-gray-300 font-medium">Pending Review</span>
              </div>
              <span className="text-2xl font-bold text-amber-600 dark:text-amber-400">{stats.pendingProducts.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                ...
                recentProducts.map((product) => (
                  <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-white max-w-md truncate">{product.title}</td>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{product.brand || 'N/A'}</td>
                  </tr>
                ))
              )}
            </tbody>
